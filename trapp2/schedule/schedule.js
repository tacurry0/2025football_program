var scheduleData = window.scheduleData = window.scheduleData || [
  ...
  },
  {
    ...
    "opponent": "高知ユナイテッドSC",
    "venue": "デンカビッグスワンスタジアム",
    "emblem": "https://jleague.r10s.jp/img/common/img_club_kochi.png",
    "details": ""
  },
  {
    "club": "kumamoto",
    ...
    "opponent": "レイラック滋賀FC",
    "venue": "平和堂HATOスタジアム",
    "emblem": "https://d1o3ac4rhk9lvf.cloudfront.net/emblem/2026/shiga.svg",
    "details": ""
  },
  {
    "club": "niigata",
    ...
    "opponent": "奈良クラブ",
    "venue": "ロートフィールド奈良",
    "emblem": "https://jleague.r10s.jp/img/common/img_club_nara.png",
    "details": ""
  },
  {
    "club": "kumamoto",
  ...
  },
  {
    ...
    "opponent": "FC大阪",
    "venue": "東大阪市花園ラグビー場",
    "emblem": "https://jleague.r10s.jp/img/common/img_club_fosaka.png",
    "details": ""
  },
  {
    "club": "niigata",
...
(function loadScheduleDataFiles() {
  function matchKey(match) {
    return [match.date || "", match.club || "", match.opponent || ""].join("|");
  }

  function mergeScheduleItems(items) {
    if (!Array.isArray(items)) return;

    const byKey = new Map(scheduleData.map(function (match, index) {
      return [matchKey(match), index];
    }));

    items.forEach(function (item) {
      if (!item || !item.date || !item.club || !item.opponent) return;

      const key = matchKey(item);
      if (byKey.has(key)) {
        const index = byKey.get(key);
        scheduleData[index] = Object.assign({}, scheduleData[index], item);
      } else {
        byKey.set(key, scheduleData.length);
        scheduleData.push(item);
      }
    });
  }

  window.SCHEDULE_DATA_FILES.forEach(function (path) {
    try {
      const xhr = new XMLHttpRequest();
      const cacheKey = path.indexOf("?") === -1 ? "?v=20260513b" : "&v=20260513b";
      xhr.open("GET", path + cacheKey, false);
      xhr.overrideMimeType("application/json; charset=utf-8");
      xhr.send(null);

      if (((xhr.status >= 200 && xhr.status < 300) || xhr.status === 0) && xhr.responseText) {
        mergeScheduleItems(JSON.parse(xhr.responseText));
      }
    } catch (error) {
      console.warn("Schedule data file load failed:", path, error);
    }
  });
})();
