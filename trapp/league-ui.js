...
(function (root) {
  ...
  function formMarkup(form) {
    ...
    return `<span class="st-form" aria-label="直近${form.length}試合・左から古い順">${form.map(m=>{const label={win:'勝ち',draw:'引き分け',loss:'負け'}[m.result];return `<span class="st-result ${m.result}" role="img" aria-label="${esc(`${m.date} ${m.opponent} ${m.score} ${label}`)}" title="${esc(`${m.date} ${m.opponent} ${m.score} ${label}`)}">${{win:'W',draw:'D',loss:'L'}[m.result]}</span>`;}).join('')}</span>`;
  }
