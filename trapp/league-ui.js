...
(function (root) {
  ...
  function formMarkup(form) {
    ...
    return `<span class="st-form" aria-label="直近${form.length}試合・左から古い順">${form.map(m=>{const label={win:'勝ち',draw:'引き分け',loss:'負け'}[m.result];return `<span class="st-result ${m.result}" role="img" aria-label="${esc(`${m.date} ${m.opponent} ${m.score} ${label}`)}" title="${esc(`${m.date} ${m.opponent} ${m.score} ${label}`)}">${{win:'○',draw:'△',loss:'●'}[m.result]}</span>`;}).join('')}</span>`;
  }
  function summary(rows, forms, getEmblem) {
    ...
    return `<section class="st-summary" data-club="${club(own)}" aria-label="${esc(own.team)}の順位">${emblem?`<img class="st-watermark" src="${esc(emblem)}" alt="">`:''}<div class="st-summary-main">${emblem?`<img class="st-summary-crest" src="${esc(emblem)}" alt="${esc(own.team)}">`:''}<div class="st-summary-rank"><span>${esc(short(own))}</span><div><strong>${value(own.rank)}</strong><small>位</small></div></div><div class="st-summary-points"><span>勝点</span><strong>${value(own.points)}</strong></div><span class="st-summary-gap">${label}</span></div><div class="st-summary-form"><span>${form.length?`直近${form.length}試合`:'直近の結果'}</span>${formMarkup(form)}</div></section>`;
  }
  function tableMarkup(rows, forms, archive, getEmblem) {
    ...
    return `<table class="st-table"><caption class="st-sr-only">${archive?'保存済み順位表':league.toUpperCase()+' 順位表'}</caption><colgroup><col class="st-col-rank"><col class="st-col-team"><col class="st-col-points"><col class="st-col-played"><col class="st-col-diff"></colgroup><thead><tr>${columns.map(([key,label])=>`<th scope="col" aria-sort="${sort.key===key?(sort.dir===1?'ascending':'descending'):'none'}"><button type="button" data-sort="${key}" aria-label="${label}で並べ替え">${label}${sort.key===key&&key!=='rank'?`<span aria-hidden="true">${sort.dir===1?'↑':'↓'}</span>`:''}</button></th>`).join('')}</tr></thead><tbody>${sorted.map(row=>{
      ...
      return `<tr class="st-team-row ${club(row)?'st-own st-'+club(row):''}"><td class="st-rank">${value(row.rank)}</td><td class="st-team"><button type="button" class="st-team-link" data-team="${esc(row.team)}" title="${esc(row.team)}" aria-label="${esc(row.team)}の公式サイト">${emblem?`<img src="${esc(emblem)}" alt="" loading="lazy">`:''}<span>${esc(short(row))}</span></button><div class="st-detail" ${expanded?'':'hidden'}>${archive?'':`<div class="st-row-form" data-form-team="${esc(row.team)}">${formMarkup(form)}</div>`}<span class="st-record">${details}<span> · 得点${value(row.goals_for)} 失点${value(row.goals_against)}</span></span></div></td><td class="st-points"><strong style="--points-fill:${Math.max(0,Math.min(100,(number(row.points)??0)/maxPoints*100))}%">${value(row.points)}</strong></td><td>${value(row.played)}</td><td>${diff(row.goal_diff)}</td></tr>`;
    }).join('')}</tbody></table>`;
  }
