...
window.TrappStandings = (function () {
  ...
  function render(container, payload, options = {}) {
    ...
    container.classList.toggle("league-expanded", expanded);
    const draw = async () => {
      ...
      const table = (name, rows) => {
        ...
        return `<h2 class="standings-group-title">${escape(name)}</h2><div class="league-table-scroll"><table class="standings-table"><thead><tr>${cols.map(([key,label]) => `<th scope="col" data-column="${key}" aria-sort="${key === sort.key ? sort.dir === 1 ? 'ascending' : 'descending' : 'none'}"><button type="button" data-sort="${key}">${label}${key === sort.key ? sort.dir === 1 ? ' ↑' : ' ↓' : ''}</button></th>`).join('')}</tr></thead><tbody>${sorted.map(row => `<tr class="${String(row.team).includes('新潟') ? 'standing-niigata' : String(row.team).includes('熊本') ? 'standing-kumamoto' : ''}">${cols.map(([key]) => key === 'team' ? `<td data-column="team" class="standing-team"><button type="button" class="league-team-link" data-team="${escape(row.team)}">${options.getEmblem?.(row.team) ? `<img class="standing-team-emblem" alt="" src="${escape(options.getEmblem(row.team))}">` : ''}<span>${escape(row.team)}</span></button><small class="league-mobile-detail">${escape(row.won)}勝 ${archive ? `${escape(row.pk_won)}PK勝 ${escape(row.pk_lost)}PK負` : `${escape(row.drawn)}分`} ${escape(row.lost)}敗<br>得点 ${escape(row.goals_for)} ／ 失点 ${escape(row.goals_against)}</small></td>` : `<td data-column="${key}" class="${key === 'points' ? 'col-pts' : key === 'rank' ? 'col-rank' : ''}">${escape(row[key] ?? '—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
      };
      const warning = archive ? '百年構想リーグの保存分です。最終順位ではありません。' : source?.stale ? '更新に失敗したため、保存済みデータを表示しています。' : '';
      container.innerHTML = `<div class="league-controls"><button type="button" class="league-mobile-expand" id="standings-expand" aria-pressed="${expanded}">${expanded ? "詳細を閉じる" : "勝敗・得失点を見る"}</button><label>大会 <select id="standings-period"><option value="current" ${archive ? '' : 'selected'}>2026/27 シーズン</option><option value="archive" ${archive ? 'selected' : ''}>2026 百年構想（保存分）</option></select></label><button type="button" id="standings-refresh" ${archive ? 'disabled' : ''}>更新</button></div>${archive ? '' : `<div class="league-tabs" role="group" aria-label="リーグ選択">${['j2','j3'].map(l => `<button type="button" data-league="${l}" aria-pressed="${l === league}">${l.toUpperCase()} <span>${l === 'j2' ? '新潟' : '熊本'}</span></button>`).join('')}</div>`}<p class="league-data-status ${warning ? 'is-stale' : ''}" role="status">${escape(warning)} ${archive ? '保存日時' : 'データ取得'}: ${escape(dateLabel(source?.fetchedAt || source?.timestamp))}${!archive && source?.sourceUpdatedAt ? ` ／ 公式更新: ${escape(source.sourceUpdatedAt)}` : ''}</p>${Object.entries(groups).map(([name, rows]) => table(name, rows)).join('') || '<p class="league-empty">順位表を取得できませんでした。更新ボタンから再試行してください。</p>'}`;
      container.querySelector('#standings-expand').onclick = () => { expanded = !expanded; render(container, payload, options); };
