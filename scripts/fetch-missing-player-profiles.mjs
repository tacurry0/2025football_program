...
async function fetchWithTimeout(url, options = {}, timeoutMs = 22000) {
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        'User-Agent': 'Mozilla/5.0 missing-profile-generator/1.0',
...
function finalTeamMatchesClub(team, club) {
  const teamNormalized = normalizeTeam(team);
  const targets = CLUB_CONFIG[club]?.finalTeamNames || [];
  return targets.some(target => teamNormalized === normalizeTeam(target));
}

function existingHasPlayer(clubJson, playerName) {
  return Object.prototype.hasOwnProperty.call(clubJson, playerName);
}

function makeSupplementProfile(profile, indexPlayer, missingName, club, matchReason) {
  const matchedAnnualRecords = profile.annual_records.filter(row => teamMatchesClub(row.team, club));
  return {
    ...profile,
    app_player_name: missingName,
    club,
    matched_annual_records: matchedAnnualRecords,
    supplement: {
      source_script: 'scripts/fetch-missing-player-profiles.mjs',
      match_reason: matchReason,
      note: matchReason === 'matched_by_final_team_only'
        ? '年度別成績に対象クラブは出ていないが、J.LEAGUE Data Site全選手一覧/詳細の最終所属が対象クラブのため未出場・百年構想リーグ補完として採用。'
        : '年度別成績で対象クラブ所属を確認。'
    },
    history_hint: {
...
async function resolveMissingPlayer(name, club, masterByName) {
  ...
  for (const indexPlayer of candidateMap.values()) {
    ...
    const annualMatch = profile.annual_records.some(row => teamMatchesClub(row.team, club));
    const finalMatch = finalTeamMatchesClub(profile.final_team || indexPlayer.final_team, club);
    if (annualMatch) {
      profiles.push(makeSupplementProfile(profile, indexPlayer, name, club, 'matched_by_annual_records'));
    } else if (finalMatch) {
      profiles.push(makeSupplementProfile(profile, indexPlayer, name, club, 'matched_by_final_team_only'));
    }
  }
  return profiles;
}
...
async function main() {
  ...
  const newMissing = {
    _meta: {
      ...
      note: 'Names only. Remaining unresolved after missing-profile supplement.'
    }
  };
  const summary = {};
  ...
  for (const club of clubs) {
    ...
    newMissing[club] = [];
    summary[club] = { total_missing_input: names.length, added: 0, still_missing: 0, skipped_existing: 0, duplicate_profile_names: 0 };
    ...
    for (let i = 0; i < names.length; i += 1) {
      ...
      if (existingHasPlayer(clubJson, name)) {
      ...
      try {
        const profiles = await resolveMissingPlayer(name, club, masterByName);
        if (profiles.length) {
          putProfile(clubJson, name, profiles);
          summary[club].added += 1;
          if (profiles.length > 1) summary[club].duplicate_profile_names += 1;
          console.log(profiles.length === 1 ? `ADD player_id=${profiles[0].player_id} ${profiles[0].supplement.match_reason}` : `ADD ${profiles.length} profiles`);
        } else {
    ...
    clubJson._meta = {
      ...(clubJson._meta || {}),
      supplemented_at: generatedAt,
      supplement_source: 'scripts/fetch-missing-player-profiles.mjs'
    };
